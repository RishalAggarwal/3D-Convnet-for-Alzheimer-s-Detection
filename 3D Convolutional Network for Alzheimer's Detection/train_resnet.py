...
model=ResnetBuilder.build_resnet_34((1,106,106,120),3)
callbacks_list=[]
#callbacks - csvlogger,checkpoint,reduce learning rate on plateau
callbacks_list.append(keras.callbacks.ModelCheckpoint(filepath='drive/model/model.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_acc', verbose=1, save_best_only=True, mode='auto', period=10))
callbacks_list.append(keras.callbacks.CSVLogger('drive/model/training.log', separator=',', append=False))
callbacks_list.append(keras.callbacks.ReduceLROnPlateau(monitor='val_acc', factor=0.1, patience=100, verbose=0, mode='auto', min_delta=0.0001, cooldown=0, min_lr=0))
model.compile('Adam','categorical_crossentropy',metrics=['accuracy'])
model.fit(x=datanp,y=truenp,batch_size=8,epochs=30,verbose=1,callbacks=callbacks_list,validation_data=(datanptst,truenptst),shuffle=True, class_weight=None, sample_weight=None, initial_epoch=0, steps_per_epoch=None, validation_steps=None)
